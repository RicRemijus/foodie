...
const CartPage = () =>{
    ...
    return(
       ...
       <Head>
        ...
        <meta name="cartpage" content="page to display the list of items in cart" />
        <link rel="icon" href="/favicon.ico" />
                ...
                <tr>
                    <th><h4>NUMBER OF ITEMS : <strong>{getItemsCount()}</strong></h4></th>
                    <th><h4 className={styles.gt}>TOTAL : <strong>&#x20A6; {getTotalPrice()}</strong></h4></th>
                    <th>
                        <Link href="/" style={{float:'right'}} ><button className={styles.checkoutBtn}>CHECKOUT</button></Link>
                    </th>
                </tr>
            </table> 
                 </div>
                   ...
                   {cart.map((item) => ( 
                    <tr className={styles.tr}>
                        ...
                        <td className={styles.td}>&#x20A6; {item.quantity*item.price}</td>
                    </tr>
                    ))}
                    </table>   
                    <h2 className={styles.gtl}>Grand Total : &#x20A6; {getTotalPrice()}</h2>
                    </>             
                 )}
            </div>
    );
